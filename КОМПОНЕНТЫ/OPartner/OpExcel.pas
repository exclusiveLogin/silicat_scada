...
uses Windows, Messages, SysUtils, Classes, stdctrls, Graphics, Controls, Forms,
  ComObj, Dialogs, ActiveX, OpXL2K, OpShared, OpModels, OpOfc2K,
  OleCtrls {$IFDEF VERSION6}, Variants {$ENDIF};

type
  {: Use ExcelRange to avoid name clashes with Word Ranges}
  ExcelRange = type OpXL2K.Range;
  {: Use ExcelWindow to avoid name clashes with Word Windows}
  ExcelWindow = type OpXL2K.Window_;
...
begin
  Intf := (RootComponent as TOpExcel).Server.Workbooks.Open(
...
begin
  AsWorkbook.SaveAs(FileName,EmptyParam,EmptyParam,EmptyParam,EmptyParam,
