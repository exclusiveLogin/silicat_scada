...
procedure TDBGridInplaceEdit.MouseDown(Button: TMouseButton; Shift: TShiftState;
  X, Y: Integer);
begin
  if (Button = mbLeft) and (FEditStyle in [esEllipsis..esDateCalendar,esDropDown]) and
...
begin
  WasPressed := FPressed;
  StopTracking;
  if (Button = mbLeft) and (FEditStyle in [esEllipsis,esDropDown]) and WasPressed then
    TCustomDBGridEh(Grid).EditButtonClick;
  inherited MouseUp(Button, Shift, X, Y);
end;
...
begin
  ...
  if EditStyle <> esSimple then
    if EditStyle in [esDataList, esPickList, esDateCalendar, esDropDown] then { esDropDown }
    begin
...
var AColumn: TColumnEh;
    ...
    CanChange:Boolean;
begin
  if (EditStyle = esDataList) and (Key in [#32..#255]) and
      TCustomDBGridEh(Grid).AllowedOperationUpdate then begin
...
begin
  ...
  if SortIndex <> 0 then begin
    ...
    if (FColumn.GetGrid.SortMarkedColumns.Count > 1) then begin
...
{  Designer := FindRootDesigner(Self);
  ...
  if (Form <> nil) and (Form.Designer <> nil) then Form.Designer.Notification(TComponent(FFooters),opRemove);
...
procedure TColumnEh.SetImageList(const Value: {$IFDEF EH_LIB_4}TCustomImageList{$ELSE}TImageList{$ENDIF});
begin
  FImageList := Value;
  if GetGrid <> nil then
    GetGrid.Invalidate;
...
begin
  ...
  end else if Field.DataType in [ftSmallint,ftInteger,ftWord,ftFloat,ftCurrency,ftBCD] then
  begin
    if Field.IsNull
...
begin
  ...
  if Result then
   Result := Result and AField.CanModify and
             (not AField.IsBlob or Assigned(AField.OnSetText)) and
...
begin
  if Index < Footers.Count then
    Result := Footers[Index]
  else
    Result := Footer;
end;
...
begin
  Current := CurrentRow;
  if (Length(Current) = 0) or {allow select new rec} (Find(Current, Index) = Value) then Exit;
  if Value then
  ...
  //ddd
  if (FGrid.Selection.FSelectionType <> gstRecordBookmarks) and (Count > 0) then begin
...
{$endif}
  ...
  try
    ...
    FIndicators.AddMasked(Bmp, clNone);
//ddd
...
begin
  Result := (LayoutLock = 0) and inherited CanEditShow and not FLockEditShow;
  //ddd
...
var
  ...
  var curLeaf: THeadTreeNode;
     ...
     drawRec,drawRec1{, FrozenRectCell}:TRect;
     OldColor:TColor;
     ThreeDLine:Integer;
  begin
...
//1.51    leftM := 0;
    ...
    {if (DHRect.Left < ColWidths[0]) and (dgIndicator in Options) then begin
      leftM := DHRect.Left - ColWidths[0];   drawRec.Left := ColWidths[0];
...
{    if (FrozenCols > 0) then begin
    ...
    if (gdFixed in AState) and (dghFixed3D in OptionsEh) then begin
...
//    if(leftM  <> 0) then
//      WriteTextEh(Canvas, drawRec, True, FrameOffs-1, FrameOffs, ALeaf.Host.Text, taCenter,tlCenter,True,AEndEllipsis,leftM,RightM)
//    else
//      Canvas.FillRect(drawRec);
      drawRec1 := drawRec;
      if(leftM  = 0) then begin
      ...
      end else Inc(RightM,2);

      WriteTextEh(Canvas, drawRec1, False, 0, FrameOffs, ALeaf.Host.Text, taCenter,tlCenter,True,AEndEllipsis,leftM,RightM);
    ...
    begin
      InflateRect(drawRec, 0, 1);
      if Highlight then
        ThreeDLine := BDR_SUNKENINNER
      else
        ThreeDLine := BDR_RAISEDINNER;
      if(leftM  <> 0) then
        DrawEdge(Canvas.Handle, drawRec, ThreeDLine, BF_TOP)
      else
        DrawEdge(Canvas.Handle, drawRec, ThreeDLine, BF_TOPLEFT);
      if(RightM  = 0) then
        DrawEdge(Canvas.Handle, drawRec, ThreeDLine, BF_BOTTOMRIGHT)
      else
        DrawEdge(Canvas.Handle, drawRec, ThreeDLine, BF_BOTTOM);
//      InflateRect(DHRect, 1, 1);
    end;
  ...
  procedure DrawClipped(imList:{$IFDEF EH_LIB_4}TCustomImageList{$ELSE}TImageList{$ENDIF}; ACanvas:TCanvas; ARect:TRect;
                        Index: Integer; Align:TAlignment);
  var CheckedRect,AUnionRect:TRect;
      OldRectRgn,RectRgn:HRGN;
      r,x,y:Integer;
  begin
    case Align of
      taLeftJustify: x := ARect.Left + LeftMarg;
      taRightJustify: x := ARect.Right - imList.Width + LeftMarg;
    else
      x := (ARect.Right + ARect.Left - imList.Width) div 2 + LeftMarg;
    end;
    ...
    if EqualRect(AUnionRect,ARect) then // ARect containt image
      ...
      OldRectRgn := CreateRectRgn(0,0,Width,Height);
...
//\\\
      ...
      begin
        ...
        if MultiSelected then
          FIndicators.BkColor := RGB(64,64,64)
        else
          FIndicators.BkColor := FixedColor;
...
// new --
         ...
         if (FUseMultiTitle = True) then begin
           ...
           TitleText := Caption;
         if (TitleImages <> nil) and (ImageIndex <> -1) then begin
           TitleText := '';
         ...
         if SortMarkerIdx <> -1 then begin
           ...
           if ARect1.Right < ARect1.Left + ImageWidth then begin
         ...
         if (FUseMultiTitle = True) then begin
           ...
           WriteTextEh(Canvas, ARect1, False, FrameOffs, FrameOffs, TitleText, taCenter,tlCenter,True,EndEllipsis,LeftMarg,RightMarg);
           //Canvas.Pen.Color := clWindowFrame;
         end
         else if (TitleHeight <> 0) or (TitleLines <> 0) then
           WriteTextEh(Canvas, ARect1, False, FrameOffs, FrameOffs, TitleText, Alignment,tlCenter,True,EndEllipsis,LeftMarg,RightMarg)
         else begin
           ARect1.Left := ARect1.Left + LeftMarg;
           ARect1.Right := ARect1.Right - RightMarg;
           WriteTextEh(Canvas, ARect1, False, FrameOffs, FrameOffs, TitleText, Alignment,tlTop,False,EndEllipsis,LeftMarg,RightMarg);
         end;
         if (TitleImages <> nil) and (ImageIndex <> -1) then begin
           with TitleImages do begin
...
//                           (ARect1.Bottom + ARect1.Top - Height) div 2, ImageIndex);
             if FUseMultiTitle then
               DrawClipped(TitleImages,Canvas,ARect1,ImageIndex,taCenter)
             else
               DrawClipped(TitleImages,Canvas,ARect1,ImageIndex,Alignment);
           end;
         end;
         if SortMarkerIdx <> -1 then begin
           ...
           if SMTMarg <> 0 then begin
             Canvas.Font := SortMarkerFont;
             if Highlight = True then Canvas.Font.Color := clWhite;
...
//\\
        ...
        begin
          ...
          if (DrawColumn.AlwaysShowEditButton) then begin
            ...
            if (AEditStyle <> esSimple) then begin
              ...
              if The3DRect then InflateRect(ARect1, 1, 1);
  ...
  begin
    InflateRect(ARect, 1, 1);
    if {((ACol < 0) or (ARow < 0)) and} Highlight then begin
...
function TCustomDBGridEh.HighlightCell(DataCol, DataRow: Integer;
  const Value: string; var AState: TGridDrawState): Boolean;
//var
//  Index: Integer;
begin
  Result := False;
...
begin
  ...
  {ddd\\};
  if (Selection.SelectionType <> gstNon) {ddd//}and (dghClearSelection in OptionsEh) {ddd\\}then
  begin
    Selection.Clear;
    Invalidate;
  end;
  ...
  begin
    ...
    begin
      ...
      if EOF and CanModify and (not ReadOnly) and
         (dgEditing in Options) and (alopAppendEh in AllowedOperations) then
        Append;
    end;
  end;
    ...
    begin
      ...
      case Key of
        VK_UP, VK_PRIOR: {d/} if (ssShift in Shift) and (dgMultiSelect in Options) then
                                 DoSelection(True,-FDatalink.ActiveRecord,False,False)
                               else {d\} MoveBy(-FDatalink.ActiveRecord);
        VK_DOWN, VK_NEXT: {d/} if (ssShift in Shift) and (dgMultiSelect in Options) then
                                 DoSelection(True,FDatalink.BufferCount - FDatalink.ActiveRecord - 1,False,False)
...
//  inherited FixedCols := FIndicatorOffset + FrozenCols + FirstInvisibleColumns;
  if not FDataLink.Active and (Columns.State = csDefault) then
    inherited FixedCols := FIndicatorOffset + FirstInvisibleColumns
  else
    inherited FixedCols := FIndicatorOffset + FrozenCols + FirstInvisibleColumns;
...
begin
  ...
  begin
    ...
    if GetCursor = Screen.Cursors[crDownCurEh] then begin //columns selection
      InvalidateCol(Cell.X);
      FDBGridEhState := dgsColSelecting;
      ResetTimer(60);
      //tmpSetTimer(Handle, 1, 60, nil);
      if ssShift in Shift then
        Selection.Columns.SelectShift(Columns[Cell.X - IndicatorOffset],False)
      else if ssCtrl in Shift then
        Selection.Columns.InvertSelect(Columns[Cell.X - IndicatorOffset])
      else begin
        Invalidate;
    ...
    begin
      ...
      if (Cell.X < FIndicatorOffset) and (dgMultiSelect in Options) and
         (Cell.Y <> -1) and FDatalink.Active then
          ...
          try
            ...
            begin
              ...
              if not (ssCtrl in Shift) and not (ssShift in Shift) and (dghClearSelection in OptionsEh) and
                ...
                ClearSelection;
              if (X >= FIndicatorOffset) and (dgMultiSelect in Options) and
                 (Button = mbLeft) and not (DataSource.DataSet.Eof and DataSource.DataSet.Bof) then begin
                if ssShift in Shift then
            ...
            //\\\
            if (dgMultiSelect in Options) {ddd//}
                 and ((dgRowSelect in Options) or (X < FIndicatorOffset)) then
              with FBookmarks do
              begin
                FSelecting := False;
...
(*  if (Cell.Y = TitleOffset-1) and (Cell.X > IndicatorOffset-1) then begin
    ARect := CellRect(Cell.X,Cell.Y);
    if ((FUseMultiTitle = False) or ((FUseMultiTitle = True) and
         not PtInRect(Rect(ARect.Left, ARect.Top, ARect.Right, ARect.Bottom -
               FLeafFieldArr[Cell.X-IndicatorOffset].FLeaf.Height + 1),Point(X, Y)))) then begin
      if ((FHintCell.X <> Cell.X) or (FHintCell.Y <> Cell.Y)) and
          ((Columns[RawToDataColumn(Cell.X)].Title.Hint <> '') or (FHintCell.X <> -2)) then begin
  ...
  if not Hinted then begin
    if (FHintCell.X <> -2) then begin
      Application.CancelHint;
      FHintCell.X := -2;
    end;
  end;*)
  if FSelectedCellPressed = True then begin
...
{$ENDIF}
  ...
  if (FTracking) and (FPressedCol <> -1) then begin
    TrackButton(X, Y);
    if (Abs(FDownMousePos.X - X) > 3) and (dgColumnResize in Options) then begin
      StopTracking;
...
//      if csClickEvents in ControlStyle then Include(ControlState, csClicked);
  ...
  if (FIndicatorPressed or FDataTracking or (FDBGridEhState = dgsRectSelecting)) {and not (FDBGridEhState = ghsRectSelecting)} then begin
//    X1 := X; Y1 := Y;
    if X1 < 0 then X1 := 0;
    ...
    if (Y > DataRect.Top) and (Y < DataRect.Bottom) then begin
      ...
      if (Cell.Y < Row) then
         if (FDBGridEhState = dgsRectSelecting) then begin
           FDatalink.Dataset.MoveBy(Cell.Y - Row);
           Selection.Rect.Select(RawToDataColumn(Cell.X),DataSource.DataSet.Bookmark,AddSel);
         end else
           DoSelection(WithSeleting and AddSel, Cell.Y-Row,False,True)
  ...
  if FDBGridEhState = dgsColSelecting then begin
    Cell := MouseCoord(X, Y);
    if (X > DataRect.Left) and (X < DataRect.Right) and (Cell.X <> -1) then
      if (ssCtrl in Shift) {and (Selection.Columns.IndexOf(Columns[RawToDataColumn(Cell.X)]) = -1)} then
        Selection.Columns.SelectShift(Columns[RawToDataColumn(Cell.X)],True)
      else
        Selection.Columns.SelectShift(Columns[RawToDataColumn(Cell.X)],False)
    else
      TimerScroll;
  end
  else if FDBGridEhState = dgsColSizing then begin
    DrawSizingLine(GridWidth, GridHeight); { XOR it out }
    FSizingPos := X + FSizingOfs;
    DrawSizingLine(GridWidth, GridHeight); { XOR it back in }
  end;
  {
  else if FDBGridEhState = ghsRectSelecting then begin
    Cell := MouseCoord(X, Y);
    BeginUpdate(); //Prevent redraw grid
    try
      if (Y >= FTitleOffset) and (Cell.Y - Row <> 0) then
         FDatalink.Dataset.MoveBy(Cell.Y - Row);
    finally
      EndUpdate;
    end;
    if X >= FIndicatorOffset then
      MoveCol(Cell.X,0);
    if (X > DataRect.Left) and (X < DataRect.Right) and (Cell.X <> -1) then
      Selection.Rect.Select(RawToDataColumn(Cell.X),DataSource.DataSet.Bookmark,True);
  end};
...
begin
  if FDBGridEhState = dgsColSizing then begin
    DrawSizingLine(GridWidth, GridHeight);
    NewSize := 0;
    ...
    for I := IndicatorOffset to FSizingIndex - 1 do
...
//ddd
  ...
  if (FPressedCell.X = Cell.X) and (FPressedCell.Y = Cell.Y) and
    ...
    (Columns[Cell.X - IndicatorOffset].GetColumnType in [ctKeyImageList..ctCheckboxes]) then
    if not Columns[Cell.X - IndicatorOffset].DblClickNextVal and not (ssDouble in Shift) and
...
*)
  if Select and not (dgRowSelect in Options) then
    if Selection.FSelectionType <> gstRectangle then
      Selection.Rect.Select(RawToDataColumn(Col),Datalink.Datasource.Dataset.Bookmark,True);
  FDatalink.UpdateData;
  if RawCol >= ColCount then
    RawCol := ColCount - 1;
  if RawCol < {//dddFIndicatorOffset} FIndicatorOffset {\\\} then RawCol := {//dddFIndicatorOffset} FIndicatorOffset {\\\};
  if Direction <> 0 then
  begin
    while (RawCol < ColCount) and (RawCol >= {//dddFIndicatorOffset} FIndicatorOffset) and
      (ColWidths[RawCol] <= 0) do
      Inc(RawCol, Direction);
    if (RawCol >= ColCount) or (RawCol < {//dddFIndicatorOffset} FIndicatorOffset) then Exit;
  end;
  OldCol := Col;
  if RawCol <> OldCol then
  begin
...
begin
  ...
  begin
    ...
    else if (AComponent is {$IFDEF EH_LIB_4}TCustomImageList{$ELSE}TImageList{$ENDIF}) then begin
...
begin
  ...
  ClearSelection;//newFBookmarks.Clear;
...
//\\\
    ...
    if dgMultiSelect in (FOptions - Value) then ClearSelection; //newFBookmarks.Clear;
...
begin
  if (csDesigning in ComponentState) and ((FDataLink = nil) or
     ((Columns.State = csDefault) and
      (FDataLink.DefaultFields or (not FDataLink.Active)))) then begin
    Windows.SetCursor(LoadCursor(0, IDC_ARROW));
    Exit;
  end;
...
//ddd
  ...
  begin
    ...
    if (Cell.X >= 0) and (Cell.X < FIndicatorOffset) and (Cell.Y > TitleOffset-1) and
        FDatalink.Active and not ( DataSource.DataSet.Eof and DataSource.DataSet.Bof) then
      Windows.SetCursor(Screen.Cursors[crRightCurEh])
    else
    if (Cell.Y = TitleOffset-1) and (Cell.X > IndicatorOffset-1) and
     not (dgRowSelect in Options) then begin
      ARect := CellRect(Cell.X,Cell.Y);
      if (HitTest.Y <= ARect.Bottom) and
        ...
        Windows.SetCursor(Screen.Cursors[crDownCurEh])
      else inherited;
    end else inherited;
  end
  else inherited;
...
begin
  ...
  th := iif(htLast.Width>0,
         ...
         Length(s), rec, DT_WORDBREAK or DT_CALCRECT), 0);
...
//  while (I <= Length(S)) and not(S[I] in WordDelims) do begin
    while (I <= Length(S))
        and not ((ByteType(S,I)=mbSingleByte) and (S[I] in WordDelims)) do
    begin
  ...
  case Footer.ValueType of
    ...
    begin
      ...
      if Assigned(DataSource) and Assigned(DataSource.DataSet)
       then if Footer.FieldName <> '' then
         Field := DataSource.DataSet.FindField(Footer.FieldName)
       else
         Field := DataSource.DataSet.FindField(Column.FieldName);
      if Field = nil then Exit; 
      with Field do begin
        v := SumList.SumCollection.GetSumByOpAndFName(goSum,FieldName).SumValue;
        case DataType of
    ...
    fvtFieldValue: if Assigned(DataSource) and Assigned(DataSource.DataSet) and
                        DataSource.DataSet.Active and (Footer.FieldName <> '') then
                      Result := DataSource.DataSet.FieldByName(Footer.FieldName).DisplayText;
...
//    Point := FMousePos;
    ADataRect := DataRect;
    if Point.X > ADataRect.Right then begin
      ...
      if LeftCol + VisibleColCount {+ FixedCols - 1} < ColCount then begin
        LeftCol := LeftCol + 1;
        if LeftCol + VisibleColCount + FixedCols < ColCount then
          Selection.Columns.SelectShift(Columns[RawToDataColumn(LeftCol + VisibleColCount+1)],True)
        else
          Selection.Columns.SelectShift(Columns[RawToDataColumn(LeftCol + VisibleColCount-1)],True);
      end
    ...
    else if Point.X < ADataRect.Left then begin
      ...
      if LeftCol > FixedCols then begin
        ...
        if LeftCol > FixedCols then
          Selection.Columns.SelectShift(Columns[RawToDataColumn(LeftCol-1)],True)
        else
          Selection.Columns.SelectShift(Columns[RawToDataColumn(LeftCol)],True);
      end
      else
    ...
    DeltaX := 0;
    DistanceX := 0;
    if FDownMousePos.X < ADataRect.Left then begin
      DeltaX := -1;
      DistanceX := ADataRect.Left - FDownMousePos.X;
    end;
    if FDownMousePos.X >= ADataRect.Right then begin
      DeltaX := 1;
...
//        end else
      if FDBGridEhState <> dgsRowSelecting then
        if DeltaX > 0 then
          MoveCol(Col+DeltaX,1,False)
        else
          MoveCol(Col+DeltaX,-1,False);
      if (FDBGridEhState = dgsRectSelecting) then begin
           FDatalink.Dataset.MoveBy(iif(Distance div 6 > 8,8,Distance div 6)*Delta);
           if (DeltaX < 0) and (LeftCol = FixedCols) then
             Selection.Rect.Select(RawToDataColumn(IndicatorOffset),DataSource.DataSet.Bookmark,True)
           else
             Selection.Rect.Select(RawToDataColumn(Col),DataSource.DataSet.Bookmark,True)
      end;
...
begin
  if FTimerActive = False then
    SetTimer(Handle, 1, Interval, nil)
  else if Interval <> FTimerInterval then begin
    StopTimer;
    SetTimer(Handle, 1, Interval, nil);
...
begin
  ...
  if FIndicatorPressed or FDataTracking or
    (FDBGridEhState =  dgsColSelecting) then TimerScroll;
end;
...
var Cell:TGridCoord;
  ...
  begin
    ...
    if (NewRect.Right-NewRect.Left > ARect.Right-ARect.Left-2-RightIndent) or
       (NewRect.Bottom-NewRect.Top > ARect.Bottom-ARect.Top-3) then
      Result := True;
  end;
begin
  inherited;
  ...
  begin
    ...
    begin  // Title hint
      ...
      Message.HintInfo^.CursorRect := CellRect(Cell.X,Cell.Y);
            ...
            ((Cell.Y >= TitleOffset) and (Cell.Y < DataRowCount + TitleOffset) and DataLink.Active and
              Column.ToolTips)) and {$IFDEF EH_LIB_4} (Mouse.Capture = 0) and {$ENDIF} (GetKeyState(VK_CONTROL) >= 0) then
    begin  // Title tooltips
      ...
      TextWider := CheckHintTextRect(s,ARect,WordWrap,TextWidth);

      with PHintInfo(Message.HintInfo)^ do
        if TextWider or ((Alignment = taRightJustify) and (DataRight - 2 > ARect.Right ))then
        begin
          HintStr := s;
          CursorRect := ARect;
          case Alignment of
            taLeftJustify:  HintPos := ClientToScreen(Point(ARect.Left-1,ARect.Top-1));
            taRightJustify: HintPos := ClientToScreen(Point(DataRight + 1 - TextWidth - 7,ARect.Top-1));
            taCenter:  HintPos := ClientToScreen(Point(DataRight + 1 - TextWidth - 6 + TextWidth div 2 - (DataRight - ARect.Left - 4) div 2, ARect.Top-1));
          end;
...
procedure TCustomDBGridEh.SetTitleImages(const Value: {$IFDEF EH_LIB_4}TCustomImageList{$ELSE}TImageList{$ENDIF});
begin
  FTitleImages := Value;
  if Value <> nil then Value.FreeNotification(Self);
  LayoutChanged;
...
begin
  ...
  for I := 0 to TitleOffset-1 do begin
  ...
  if (Line > HorzGridBoundary) and (HorzGridBoundary = ClientWidth) and (X >= ClientWidth - Back)
...
begin
  ...
  if (Value <= FixedCols-1) and (Value >= IndicatorOffset) then begin
    {FFrozenFocuse := True;
    FFrozenCol := Value;}
...
begin
  if FooterRowCount > 0 then
    Result := RowCount - FooterRowCount - 1 - TitleOffset
  else
    Result := RowCount - TitleOffset;
end;
...
begin
  if (cvFooterAlignment in FAssignedValues) and (Value = FAlignment) then Exit;
  FAlignment := Value;
...
begin
  if (cvFooterColor in FAssignedValues) and (Value = FColor) then Exit;
  FColor := Value;
      ...
      case ValueType of
        fvtSum: begin
                  ...
                  if FieldName <> '' then
                    ...
                    FDBSum.FieldName := FColumn.FieldName;
                end;
...
begin
  ...
  if (ASelType = gstNon) and (dgAlwaysShowEditor in FGrid.Options) then
    FGrid.ShowEditor
  else
    FGrid.HideEditor;
end;
...
begin
  case SelectionType of
    gstRecordBookmarks:
      if FGrid.SelectedRows.Count = 0 then  begin
        FSelectionType := gstNon;
        FGrid.Invalidate;
      end;
    gstRectangle:
...
begin
  for i := 0 to Count-1 do
    if ACol.Index < Items[i].Index then begin
      Insert(i,ACol);
      Exit;
    end;
  inherited Add(ACol);
...
begin
  for i := Count-1 downto 0 do begin
    ...
    for j := 0 to FGrid.Columns.Count-1 do
      if FGrid.Columns[j] = Items[i] then begin
        Found := True;
        Break;
      end;
...
//  FGrid.Invalidate; //tmp
  ...
  if FAnchor = nil then begin
    Select(ACol,True);
    FAnchor := ACol;
  end else begin
    if (FAnchor.Index < FShiftCol.Index) then
    begin
      if (FShiftCol.Index < ACol.Index) then begin
        FromIndex := FShiftCol.Index;
        ToIndex := ACol.Index;
        NeedAdd := True;
      end else if (FShiftCol.Index > ACol.Index) then begin
        FromIndex := FShiftCol.Index;
    ...
    else if (FAnchor.Index > FShiftCol.Index) then begin
      if (FShiftCol.Index > ACol.Index) then begin
        FromIndex := FShiftCol.Index;
        ToIndex := ACol.Index;
        Step := -1;
...
var OldRec:Integer;
    ...
    begin
      Result := -1;
      for i := 0 to FGrid.FDataLink.RecordCount-1 do begin
       FGrid.FDataLink.ActiveRecord := i;
       with FGrid.DataSource.DataSet do
         if CompareBookmarks(Pointer(BM),Pointer(Bookmark)) = 0 then begin
           Result := i;
           Break;
         end;
      end;
...
begin
  Result := -1;
  if CheckState then
    if FShiftCell.Col < FAnchor.Col then
      Result := FShiftCell.Col
    else
      Result := FAnchor.Col
  else
    RaiseGridError('Error in TDBGridEhSelectionRect.GetBottomRow');
...
begin
  ...
  if (FAnchor.Col = -1) or not AddSel then begin
    FAnchor.Col := ACol;
    FAnchor.Row := ARow;
    FShiftCell.Col := ACol;
    FShiftCell.Row := ARow;
