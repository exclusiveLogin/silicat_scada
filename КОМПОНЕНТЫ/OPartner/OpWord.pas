...
uses
  ...
  OpWrd2K, OpOfc2K, OpMSO, OpShared, ActiveX, OpModels, OpOutlk, OpDbOfc
  {$IFDEF VERSION6}, Variants {$ENDIF} ;

type
 //: Name conflict with Excel Window
 WordWindow = type OpWrd2K.Window_;
  ...
  if Connected then begin
    ...
    if (Item is TOpDocumentShape) then begin
      ...
      if Item.Index < (Item.ParentItem.Intf as _Document).Shapes.Count then begin
        Result := ((Item.ParentItem.Intf as _Document).Shapes.Item(OleIndex) as OpWrd2K.Shape);
        TOpDocumentShape(Item).FLeft :=
          ((Item.ParentItem.Intf as _Document).Shapes.Item(OleIndex) as OpWrd2K.Shape).Left;
        TOpDocumentShape(Item).FTop :=
          ((Item.ParentItem.Intf as _Document).Shapes.Item(OleIndex) as OpWrd2K.Shape).Top;
        TOpDocumentShape(Item).FWidth :=
          ((Item.ParentItem.Intf as _Document).Shapes.Item(OleIndex) as OpWrd2K.Shape).Width;
        TOpDocumentShape(Item).FHeight :=
          ((Item.ParentItem.Intf as _Document).Shapes.Item(OleIndex) as OpWrd2K.Shape).Height;
        TOpDocumentShape(Item).FShapeName :=
          ((Item.ParentItem.Intf as _Document).Shapes.Item(OleIndex) as OpWrd2K.Shape).Name;
      end
    end;
  end;
end;
    ...
    try
      ...
      begin
        ...
        try
          ...
          FMailMerge.AsMailMerge.OpenDataSource('' , emptyParam, emptyParam, emptyParam, VarT, VarF,
             emptyParam, emptyParam, emptyParam, emptyParam, emptyParam, ConnectionStr,
             SQLText, emptyParam);
        finally
...
begin
  ...
  begin
    ...
    AsDocument.SaveAs(OleFileName, OleFileFormat, emptyParam, emptyParam,
...
begin
  ...
  begin
    ...
    Intf:= TOpWord(RootComponent).FServer.Documents.OpenOld(DocFileVariant,
      ...
      emptyParam, emptyParam, emptyParam, emptyParam);
    {$IFDEF VERSION6}
    if (AsDocument.MailMerge.MainDocumentType) <> Int64(OpWrd2K.wdNotAMergeDocument) then
    {$ELSE}
    if (AsDocument.MailMerge.MainDocumentType) <> Integer(OpWrd2K.wdNotAMergeDocument) then
    {$ENDIF}
      FMailMerge.SetDoc(AsDocument);
...
begin
  FAddress:= Value;
  if CheckActive(False,ctProperty) then
    if (TOpWord(RootComponent).OfficeVersion = ov2000) then
...
begin
  if assigned(Intf) then (Intf as OpWrd2K.Shape).Select(emptyParam);
end;
...
begin
  if CheckActive(True,ctMethod) then
  begin
    intf:= (ParentItem.Intf as _Document).Shapes.AddShape(Integer(OpOfc2K.msoTextBox),
...
begin
  ...
  begin
    ...
    if Assigned(intf) then
    begin
      (Intf as OpWrd2K.Shape).Height:= FHeight;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if Assigned(intf) then
    begin
      (Intf as OpWrd2K.Shape).Left:= FLeft;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if Assigned(intf) then
    begin
      (Intf as OpWrd2K.Shape).Name:= FShapeName;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if Assigned(intf) then
    begin
      (Intf as OpWrd2K.Shape).Top:= FTop;
    end;
  end;
end;
...
begin
  ...
  begin
    ...
    if Assigned(intf) then
    begin
      (Intf as OpWrd2K.Shape).Width:= FWidth;
    end;
  end;
end;
